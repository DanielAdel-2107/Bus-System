...
class NotificationsHelper {
  ...
  Future<void> initNotifications() async {
    try {
      ...
      // get device token with a timeout to prevent hanging in emulators
      _firebaseMessaging.getToken().timeout(const Duration(seconds: 4)).then((deviceToken) {
        if (deviceToken != null) {
          log("✅ Device Token: $deviceToken");
        }
      }).catchError((e) {
        log("⚠️ Error or Timeout getting device token: $e");
        return null;
      });
    } catch (e) {
  ...
  Future<String?> getAccessToken() async {
    try {
      String jsonString = await rootBundle.loadString('assets/bus-system.json');
  ...
  // Save device token to user profile (array of tokens)
  ...
  Future<void> sendNotificationToUser(String userId, {required String title, required String body, String? type}) async {
    try {
      final supabase = Supabase.instance.client;
